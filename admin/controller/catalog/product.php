  	...
  	protected function getForm() {
		...
		if (isset($this->request->post['location'])) {
			...
			$this->data['location'] = $product_info['location'];
      		...
      		$this->data['location'] = '';
				...
				if (isset($this->request->post['waranty'])) {
      		$this->data['waranty'] = $this->request->post['waranty'];
    	} elseif (!empty($product_info)) {
			$this->data['waranty'] = $product_info['waranty'];
		} else {
      		$this->data['waranty'] = '0';
    	}
				if (isset($this->request->post['videolink'])) {
      		$this->data['videolink'] = $this->request->post['videolink'];
    	} elseif (!empty($product_info)) {
		...
		foreach ($products as $product_id) {
			...
			if ($related_info) {
				...
				if (isset($this->request->post['product_soput'])) {
			$productsop = $this->request->post['product_soput'];
		} elseif (isset($this->request->get['product_id'])) {		
			$productsop = $this->model_catalog_product->getProductsoput($this->request->get['product_id']);
		} else {
			$productsop = array();
		}
		...
		foreach ($productsop as $productsop_id) {
			$soput_info = $this->model_catalog_product->getProduct($productsop_id);
			
			if ($soput_info) {
				$this->data['product_soput'][] = array(
					'product_id' => $soput_info['product_id'],
					'name'       => $soput_info['name']
				);
			}
		}
