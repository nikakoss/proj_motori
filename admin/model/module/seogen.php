...
class ModelModuleSeogen extends Model {
	...
	public function generateInformation($information, $data) {
		...
		if($data['seogen_overwrite'] || is_null($information['keyword'])) {
		...
		$updates = array();
		if(isset($information['seo_h1']) && ($data['seogen_overwrite'] || (strlen(trim($information['seo_h1']))) == 0)) {
			$h1 = trim(strtr($data['informations_h1_template'], $tags));
			$updates[] = "`seo_h1`='" . $this->db->escape($h1) . "'";
		}
		if(isset($information['seo_title']) && ($data['seogen_overwrite'] || (strlen(trim($information['seo_title']))) == 0)) {
			$title = trim(strtr($data['informations_title_template'], $tags));
			$updates[] = "`seo_title`='" . $this->db->escape($title) . "'";
		}
		if(isset($information['meta_keyword']) && ($data['seogen_overwrite'] || (strlen(trim($information['meta_keyword']))) == 0)) {
			$meta_keyword = trim(strtr($data['informations_meta_keyword_template'], $tags));
			$updates[] = "`meta_keyword`='" . $this->db->escape($meta_keyword) . "'";
		}
		if(isset($information['meta_description']) && ($data['seogen_overwrite'] || (strlen(trim($information['meta_description']))) == 0)) {
