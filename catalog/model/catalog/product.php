...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'waranty'         => $query->row['waranty'],
				'videolink'         => $query->row['videolink'],
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_manufacturer_id'])) {
			...
			$sql .= "AND p.product_id IN( ".$data['prods']." ) ";
		}
	...
	public function getProductRelated($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_soput pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.soput_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
		
		foreach ($query->rows as $result) { 
			$product_data[$result['soput_id']] = $this->getProduct($result['soput_id']);
		}
