...
<ul class="table basketTable">
                <li class="tableTitleRow">
				   <span class="tableTitleCol one">фото</span>
                    <span class="tableTitleCol two">Название</span>
                    <span class="tableTitleCol three">Количество</span>
                    <span class="tableTitleCol four">Цена</span>
					 <span class="tableTitleCol five">Итого</span>
                  
                </li>
				  <?php foreach ($products as $product) { ?>
                <li class="tableItem">
                    <div class="tableItemCol one">
					       <?php if ($product['thumb']) { ?>
                    <a href="<?php echo $product['href']; ?>"><img src="<?php echo $product['thumb']; ?>" alt="<?php echo $product['name']; ?>" title="<?php echo $product['name']; ?>" /></a>
                <?php } ?>
					</div>
					<div class="tableItemCol two"><a class="itemTitle" href="<?php echo $product['href']; ?>"> <?php echo $product['name']; ?></a>
                ...
                <img src='<?php echo $simple->tpl_joomla_path() ?>catalog/view/image/plus.png' border='0' onclick="jQuery(this).prev().val(~~jQuery(this).prev().val()+1);simplecheckout_reload('cart_value_increased');">
                    ...
                    <div class="tableItemCol four"><?php echo $product['price']; ?></div>
					<div class="tableItemCol five"><?php echo $product['total']; ?></div>
					<div class="tableItemCol six"><a href="#" onclick="jQuery('#simplecheckout_remove').val('<?php echo $product['key']; ?>');simplecheckout_reload('product_removed');return false;" class="buttonClose"></a></div>
                </li>
                  <?php } ?>
            </ul>
<?php foreach ($totals as $total) { ?>   
   <div class="totalSum clearfix">
                <div class="totalSumText"><?php echo $total['title']; ?>:</div>
             
				<div class="totalSumValue"><?php echo $total['text']; ?><?php if ($total['code'] == 'coupon') { ?> <a href="#" onclick="jQuery('input[name=coupon]').val('');simplecheckout_reload('coupon_removed');return false;" class="chbuttonClose"></a>
            <?php } ?>
            <?php if ($total['code'] == 'voucher') { ?>
           <a href="#" onclick="jQuery('input[name=voucher]').val('');simplecheckout_reload('voucher_removed');return false;" class="chbuttonClose"></a>
            <?php } ?>
            <?php if ($total['code'] == 'reward') { ?>
			<a href="#" onclick="jQuery('input[name=reward]').val('');simplecheckout_reload('reward_removed');return false;" class="chbuttonClose"></a>
           <?php } ?>
...
<?php if (isset($modules['coupon']) || isset($modules['reward']) || isset($modules['voucher'])) { ?>
    <div class="simplecheckout-cart-total simplecheckout-cart-buttons">
		...
		<a class="basket button" onclick="simplecheckout_reload('cart_changed');return false;" href="#"><?php echo $button_update; ?></a>
    </div>
<?php } ?>
